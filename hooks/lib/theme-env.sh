...
# Shared runtime for hexciri theme hooks. hexciri-hook runs hooks in
# ~/.config/hexciri/hooks/theme-set.d directly, so every bundled plugin
# sources this file to load the current theme colors and helper functions.
# Ported from OldJobobo/theme-hook-plugin-manager (thpm branch).
THPM_CONFIG_FILE="${THPM_CONFIG_FILE:-${XDG_CONFIG_HOME:-$HOME/.config}/hexciri/theme-hook.toml}"
...
THPM_STATE_DIR="${THPM_STATE_DIR:-$(thpm_config_path paths state_dir "$HOME/.local/share/hexciri/theme-hook")}"
...
THPM_QUATTRO_COLORS_FILE="$HOME/.local/state/hexciri/current/theme/colors.toml"
THPM_LEGACY_COLORS_FILE="$HOME/.config/hexciri/current/theme/colors.toml"
