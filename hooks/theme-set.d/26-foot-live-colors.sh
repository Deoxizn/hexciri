...
HEXCIRI_COLORS_FILE="$THPM_CURRENT_THEME_DIR/colors.toml"
...
parse_hexciri_color() {
  local key="$1"
  local file_path="${2:-$HEXCIRI_COLORS_FILE}"
  awk -F= -v wanted="$key" '
    $1 ~ /^[[:space:]]*#/ {
      next
    }
...
load_color() {
  ...
  # terminals match new Foot windows even when Hexciri's template output
  ...
  if ! parsed="$(normalize_hex "$parsed" 2>/dev/null)"; then
    parsed="$(parse_hexciri_color "$colors_key")"
  fi
  parsed="${parsed#\#}"
  parsed="${parsed%% *}"
  if parsed="$(normalize_hex "$parsed" 2>/dev/null)"; then
...
for idx in $(seq 0 15); do
  ...
  if ! value="$(normalize_hex "$value" 2>/dev/null)"; then
    ...
    if ! value="$(normalize_hex "$parsed" 2>/dev/null)"; then
      parsed="$(parse_hexciri_color "color$idx")"
      parsed="${parsed#\#}"
      parsed="${parsed%% *}"
      if value="$(normalize_hex "$parsed" 2>/dev/null)"; then
        :
