#!/usr/bin/env bash
# Themes cliamp on an Hexciri theme switch.
#
...
# so a richer hexciri.toml is impossible without an upstream schema change.
#
...
target="$theme_dir/hexciri.toml"
MARKER="# thpm:cliamp-hexciri"
...
if [ -n "${THPM_COLORS_FILE:-}" ]; then colors_file="$THPM_COLORS_FILE"
    ...
    quattro_colors_file="$HOME/.local/state/hexciri/current/theme/colors.toml"
    legacy_colors_file="$config_root/hexciri/current/theme/colors.toml"
    if [ -n "$cf" ]; then
