...
modify_steam_theme() {
    if [[ ! -d "$adwaita_location/adwaita/colorthemes/hexciri" ]]; then
        mkdir -p "$adwaita_location/adwaita/colorthemes/hexciri/"
    fi
}
modify_install_script() {
    if ! grep -q "hexciri" "$adwaita_location/install.py"; then
        sed -i.bak 's/\("cantarell"\)/\1, "hexciri"/' "$adwaita_location/install.py"
    fi
}
...
cp -p -f "$output_file" "$adwaita_location/adwaita/colorthemes/hexciri/hexciri.css"
