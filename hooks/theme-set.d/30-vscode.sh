...
if [[ -f "$THPM_CURRENT_THEME_DIR/vscode.json" ]]; then
    ...
    #   - a MARKETPLACE id in vscode.json (Publisher.name) -> hexciri-theme-set-vscode already
    ...
    # Nothing local to install (a Marketplace id or a built-in colorTheme) -> hexciri-theme-set-vscode
...
modify_extension_manifest() {
    hexciri_entry=$(jq 'first(.contributes.themes[] | select(.label == "Hexciri"))' "$install_path/package.json")
    if [[ -z "$hexciri_entry" ]]; then
        hexciri_entry='{"label": "Hexciri", "uiTheme": "vs-dark", "path": "./themes/base16/hexciri.json"}'
        new_manifest=$(jq --argjson theme "$hexciri_entry" '.contributes.themes += [$theme]' "${install_path}/package.json")
...
install_location="$install_path/themes/base16/hexciri.json"
