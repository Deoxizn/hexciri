...
create_spicetify_styling() {
    mkdir -p "$HOME/.config/spicetify/Themes/hexciri"
cat > "$HOME/.config/spicetify/Themes/hexciri/user.css" << EOF
:root,
.encore-dark-theme,
.encore-base-set,
...
change_spicetify_theme() {
    spicetify config current_theme hexciri > /dev/null
...
cat > "$HOME/.config/spicetify/Themes/hexciri/color.ini" << EOF
[base]
main                = ${color00}
player              = ${color00}
card                = ${color00}
