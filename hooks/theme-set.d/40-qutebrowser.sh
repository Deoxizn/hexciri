...
# Ensure config.py imports the hexciri theme
if [[ ! -f "$config_file" ]]; then
    cat > "$config_file" << 'PYEOF'
config.load_autoconfig()
import hexciri.draw
hexciri.draw.apply(c)
PYEOF
elif ! grep -q 'hexciri.draw' "$config_file"; then
    sed -i '1i import hexciri.draw' "$config_file"
    echo 'hexciri.draw.apply(c)' >> "$config_file"
fi
